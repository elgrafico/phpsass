...
class SassScriptFunctions
{
    ...
    /**
     * COMPASS alias
     * Darkens the $colour by mixing it with black as specified by $percentage.
     * shade($colour, $percentage)
     *
     * @param SassColour The colour
     * @param float Percentage of black to be mixed in
     * @return new SassColour The shaded colour
     * @throws SassScriptFunctionException If $colour is not a colour
     */
    public static function shade($colour, $percentage = '50%')
    {
        if (is_object($percentage)) {
            $percentage = new SassNumber($percentage);
        }
        SassLiteral::assertType($colour, 'SassColour');
        SassLiteral::assertType($percentage, 'SassNumber');
        SassLiteral::assertInRange($percentage, 0, 100, '%');

        return self::darken($colour, $percentage);
    }

    /**
     * COMPASS alias
     * Lightens the $colour by mixing it with white as specified by $percentage.
     * tint($colour, $percentage)
     *
     * @param SassColour The colour
     * @param float Percentage of white to be mixed in
     * @return new SassColour The shaded colour
     * @throws SassScriptFunctionException If $colour is not a colour
     */
    public static function tint($colour, $percentage = '50%')
    {
        if (is_object($percentage)) {
            $percentage = new SassNumber($percentage);
        }
        ...
        return self::lighten($colour, $percentage);
    }
